...
#helm install kritis https://storage.googleapis.com/kritis-charts/repository/kritis-charts-0.1.0.tgz --set certificates.ca="$(cat ca.crt)" --set certificates.cert="$(cat kritis.crt)" --set certificates.key="$(cat kritis.key)"
